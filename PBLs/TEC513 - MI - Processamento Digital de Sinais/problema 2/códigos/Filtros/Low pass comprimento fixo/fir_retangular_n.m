clc; clear; close;
% Projeto de filtro FIR com janela retangular - Resposta Finita ao Impulso
...
wp = (fp/(fs_sinal/2))*pi; % Frequência de passagem normalizada
ws = (fs/(fs_sinal/2))*pi; % Frequência de parada normalizada
...
% Encontrando o comprimento da janela de Retangular
N = 101; % Comprimento ímpar para um filtro FIR de ordem 100. FIR tipo I.
...
window_rect = myRect(N)'; % Criando a função janela de Retangular

%Janelando o sinal
hd_sinc_window = hd_sinc.*window_rect;
...
H = tf(hd_sinc_window, 1, 1/fs_sinal);
...
% Sinc janelada para o filtro
figure(3);
stem(hd_sinc_window);
title(sprintf('Sinc janelada para o filtro (número de amostras = %d)', N));
...
audiowrite("./Audio_Retangular/SinalRuidoso_filtrado_retangular.wav", sinal_reconstruido_normalizado, fs_sinal)